...
| **`Authors`** | Optional | Text string or list (e.g. `Jim Collinsworth`, `Jim Collinsworth, LLM-Gemini3.8`) | Author credit. **Rule 12**: When AI is an author, co-author, or suspected contributor, credit the exact model ID prefixed with `LLM-` (e.g., `LLM-Gemini3.8`, `LLM-Qwen3.5`). |
...
> - **Pursuit Lanes** represent domains of human interest/pursuit: `AI`, `Health`, `Making`, `Music`.
...
*Under design (see GitHub Issue for implementation roadmap).*
