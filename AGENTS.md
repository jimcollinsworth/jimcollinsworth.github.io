...
> - Everything in `content/` (`content/posts/`, `content/pages/`, `content/ideas/`) is templated Markdown with metadata, photos, and media authored exclusively by Jim.
> - **Zero AI Content in `content/`**: No file inside `content/` contains AI-generated text, AI drafts, or AI summaries. Source Markdown files in `content/` do NOT require `[Mine]` or `[AI]` source-level tags cluttering Obsidian notes, because everything in `content/` is intrinsically Jim's.
