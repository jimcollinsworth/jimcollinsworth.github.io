...
#### Intra-Page Provenance & Pure Markdown Shortcuts
Authors and agents use clean Markdown tags anywhere in headings, paragraphs, and list items. Pelican's `ObsidianMarkdownReader` automatically compiles them into accessible SVG provenance badges:
- `[Mine]`: Original human text, summaries, photos, or physical builds authored by Jim.
- `[AI]`: Codebases, tools, haiku summaries, or explorations generated by AI.
- `[Me]`: Direct personal biodata, autobiographical notes, or health logs.
- `[Ours]`: Collaborative works created jointly with family, peers, or AI.
- `[Theirs]`: Curated external references, quotes, and third-party works.
- `[AI+Mine]`: Dual provenance for AI codebase stewardship and human direction.

*Lead intros and summaries across all site pages are authored by Jim and must be prefaced with `[Mine]`.*
